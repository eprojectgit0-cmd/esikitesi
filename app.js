...
$(document).on('click', '.product-card', function () {
    ...
    var price  = $(this).find('.product-price').text().trim();
    var cat    = $(this).find('.product-tag').text().trim();

    $('.prod-img').attr('src', img).attr('alt', name);
    $('.prod-cat-tag').text(cat);
    $('.prod-name').text(name);
    $('.prod-price').text(price);
    $('.product-item').fadeIn(250);
});
